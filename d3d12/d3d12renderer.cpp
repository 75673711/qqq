...
void D3D12RenderNode::init()
{
	...
	*vp++ = 1.0f; *vp++ = 1.0f; *vp++ = 1.0f;
	...
	//ID3D11Texture2D* myaaaa = nullptr;
	//hr = d3dDevice->CreateTexture2D(&desc1, &initData, &myaaaa);

	//HANDLE handle;
	//IDXGIResource1* pResource;
	//hr = myaaaa->QueryInterface(__uuidof(IDXGIResource1), (void**)&pResource);
	...
	hr = m_device->OpenSharedHandle((HANDLE)(0x0000000040001dc2), __uuidof(ID3D12Resource), (void**)(&share_resource));
	qDebug() << hr;
	...
	m_device->CreateShaderResourceView(share_resource, &srvDesc, m_srvHeap->GetCPUDescriptorHandleForHeapStart());
