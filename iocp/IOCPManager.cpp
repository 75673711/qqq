...
{
	static IOCPManager* ptr_ins = nullptr;
	if (ptr_ins == nullptr)
	{
		ptr_ins = new IOCPManager;
	}

	return ptr_ins;
}

IOCPManager::IOCPManager() : QObject(nullptr)
{
...
SOCKET CreateSocket(void) {
	...
	// IPPROTO_IP 当 socket type 是 SOCK_STREAM and family is AF_INET 默认tcp
...
DWORD WINAPI WorkerThread(LPVOID WorkThreadContext) {
	...
	ClientProcessor* ptr_client = NULL;
	...
	while (TRUE) {
		...
		// old----------------
		// GetQueuedCompletionStatus 返回时lpOverlapped为异步接口传入的指针 而lpIOContext的第一参数为lpOverlapped指针   
		// 所以可将该地址直接转为PER_IO_CONTEXT结构体地址
		//lpIOContext = (PPER_IO_CONTEXT)lpOverlapped;    
		//qDebug() << (int)lpOverlapped;
		//qDebug() << (int)(&lpPerSocketContext->pIOContext->Overlapped);
		// end-------------
		...
		if (lpOverlapped == &lpIOContext->recv_overlapped)
		{
			ptr_client = lpIOContext->ptr_client_;
			if (ptr_client != nullptr)
			{
				ptr_client->OnGetNewData(dwIoSize);
			}
		...
		case ClientIoAccept:
			...
			// as the option value. 
			//
			// 这里是listensocket 出发的   为accpetsocket新建一个上下文   再重新构建一个listen的accpetsocket覆盖掉原来的
			nRet = setsockopt(
			...
			if (dwIoSize) {
				...
				//);
				hRet = StringCbCopyA(lpAcceptSocketContext->pIOContext->Buffer,
					sizeof(lpPerSocketContext->pIOContext->Buffer),
					lpPerSocketContext->pIOContext->Buffer
				);
			...
			// 始终开始读取数据
			ptr_client = new ClientProcessor();
			ptr_client->Init(lpAcceptSocketContext);
			lpAcceptSocketContext->pIOContext->ptr_client_ = ptr_client;
			if (ptr_client != nullptr)
			{
				ptr_client->OnGetNewData(0);  // 为0时代表第一次建立连接完成  开始读数据
			}
...
PPER_SOCKET_CONTEXT CtxtAllocate(SOCKET sd, IO_OPERATION ClientIO) {
	...
	if (lpPerSocketContext) {
		...
		if (lpPerSocketContext->pIOContext) {
			...
			// my
			lpPerSocketContext->pIOContext->recv_overlapped.Internal = 0;
			lpPerSocketContext->pIOContext->recv_overlapped.InternalHigh = 0;
			lpPerSocketContext->pIOContext->recv_overlapped.Offset = 0;
			lpPerSocketContext->pIOContext->recv_overlapped.OffsetHigh = 0;
			lpPerSocketContext->pIOContext->recv_overlapped.hEvent = NULL;
			// end------------
...
int myprintf(const char *lpFormat, ...) {
	...
	QString temp(lpFormat);

	va_start(arglist, lpFormat);

	nLen = lstrlen(temp.toStdWString().c_str());
	hRet = StringCchVPrintf(cBuffer, 512, temp.toStdWString().c_str(), arglist);
