...
void MainWindow::on_pushButton_clicked()
{
    QString fileName = QFileDialog::getOpenFileName(this,
          tr("select exe"), "/home", tr("Exe File (*.exe)"));

           // Or otherwise load icon data

    QString iconName = QFileDialog::getOpenFileName(this,
          tr("select icon"), "/home", tr("Icon File (*.ico)"));

    const wchar_t * exe = reinterpret_cast<const wchar_t *>(fileName.utf16());
    const wchar_t * icon = reinterpret_cast<const wchar_t *>(iconName.utf16());
    qDebug() << Test((wchar_t *)exe, (wchar_t *)icon);
}

bool MainWindow::Test(wchar_t *exeFile, wchar_t *iconFile)
{
